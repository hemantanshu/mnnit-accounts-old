...
while (true){
	...
	if(!isset($_POST[$variable]))
		break;
	array_push($completeEmployeeId, $_POST[$variable]);	
}	
if(!sizeof($completeEmployeeId, 1))
	$accountHead->palert("No Employee Record Found For The Given Details", './report_maccountheads.php');

$i = 0;
$completeAccountHeadId = array();
while (true){	
	$variable = "accountHead".$i;
	++$i;
	if(!isset($_POST[$variable]))
		break;
	array_push($completeAccountHeadId, $_POST[$variable]);
}	
if(!sizeof($completeAccountHeadId, 1))
	$accountHead->palert("No AccountHead Type Has Been Selected. Please Select", './report_maccountheads.php');	
unset($variable);
...
foreach ($completeAccountHeadId as $accountHeadId){
	array_push($data[0], $accountHead->getAccountHeadName($accountHeadId));	
}
...
foreach ($completeEmployeeId as $employeeId){
	++$i;
	$data[$i] = array();	
	array_push($data[$i], $employeeId);

	foreach ($completeAccountHeadId as $accountHeadId)
		array_push($data[$i], number_format($reporting->getSalaryAllowanceInfo($employeeId, $month, $accountHeadId, false), 2, '.', ''));
}
ob_end_flush();
?>
