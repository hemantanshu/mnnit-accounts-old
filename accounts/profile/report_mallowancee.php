...
include '../include/class.reporting.php';
...
if(!sizeof($completeAllowanceId, 1))
	$allowance->palert("No Allowance Type Has Been Selected. Please Select", './report_mallowances.php');	
unset($variable);
...
foreach ($completeAllowanceId as $allowanceId){
	...
	elseif ($allowanceId == 'deduction')
		...
		array_push($data[0], $allowance->getAllowanceTypeName($allowanceId));	
}
...
foreach ($completeEmployeeId as $employeeId){
	...
	foreach ($completeAllowanceId as $allowanceId)
		array_push($data[$i], number_format($reporting->getSalaryAllowanceInfo($employeeId, $month, $allowanceId, true), 2, '.', ''));
