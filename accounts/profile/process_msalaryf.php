    ...
    <div class="main">
      <div class="content">
        ...
        <table align="center" border="0" width="100%">
            ...
            <?php 
            	if(isset ($_POST) && $_POST['submit'] == "Confirm Processing Salary Of These Employees"){
		        $i = 0;
		        while(true){
		            ...
		            if(!isset ($_POST[$employeeName])){
		                $accounts->preserveAccountHead();                
		                if($accounts->isAdmin())
		                    $accounts->palert("Salary Processed Successfully For These Employees", "./salary_process.php");
		                else
		                    $accounts->palert("Salary Processed Successfully But Is In Pending Status Till Final Consent From The Admin", "./salary_process.php");
		            }
		            if($_POST[$checkbox] == 1){
		                $employeeId = $_POST[$employeeName];
		                $days = $_POST[$monthDays];
		                $bank = $_POST[$bankTransfer];
		                
		                if(!$ePending->isEmployeeSalaryInPendingStatus($employeeId) && !$ePending->isEmployeeSalaryProcessed($employeeId))
		                        $accounts->processEmployeeMonthlySalary($employeeId, $days, $bank);
		                $personalInfo->getEmployeeInformation($employeeId, true);
		                echo "
		                	<tr>
				            	<th align=\"left\" style=\"padding-left:100px\">The Monthly Salary Has Been Processed For ".$personalInfo->getName()."</th>
				            </tr>";								
		            }            
		        }        
		    }
      ...
      <div class="sidenav">
      	<hr size="2" /><center>
        <font color="#FF0000" size="+1"><b><?php echo $accounts->getOfficerName(); ?></b></font></center>
       	<hr size="2" /><br />
